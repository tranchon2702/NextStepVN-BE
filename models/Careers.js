...
const JobSchema = new mongoose.Schema({
  ...
  seo: {
    ...
    metaDescription: {
      ...
      trim: true,
      default: function() {
        return this.description ? this.description.substring(0, 160) : '';
      }
    },
    metaKeywords: {
      type: [String],
...
JobSchema.pre('save', async function(next) {
  ...
  if (this.titleJa && (!this.slugJa || this.isModified('titleJa'))) {
    ...
    if (!baseSlugJa || baseSlugJa.length === 0) {
      // Use a simple format: job-ja-{timestamp}
      baseSlugJa = `job-ja-${Date.now()}`;
    }
    ...
    let counter = 2;
    const JobModel = this.constructor;
    while (await JobModel.findOne({ slugJa: uniqueSlugJa, _id: { $ne: this._id } })) {
